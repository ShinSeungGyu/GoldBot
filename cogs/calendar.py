    ...
    async def broadcast_embed(self, embed, chname="로아-골드섬"):
        ...
        for guild in self.bot.guilds:
            target_channel = discord.utils.get(
                guild.text_channels, name=chname)
            if target_channel:
                # 개별 전송을 코루틴으로 생성
...
# ----------------------------------------
            ...
            try:
                ...
                if delay > 0:
                    self.bot.loop.create_task(
                        self.scheduled_island_alert(delay, name, event_time))
            except Exception as e:
                print(f"파싱 에러: {e}")
    ...
    async def scheduled_island_alert(self, delay, island_name, event_time):
        ...
        embed.add_field(
            name="시작 시간", value=f"🕒 {event_time.strftime('%H:%M')}", inline=False)
    ...
    async def check_voyage_times(self):
        ...
        if not voyage_data:
            ...
            # 리스트 내용을 합쳐서 필드에 추가
            voyage_list = "\n".join(voyage_data)
            embed.add_field(name="오늘의 항해 일정",
                            value=f"```\n{voyage_list}\n```", inline=False)
