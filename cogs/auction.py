...
class AccessorySearchView(ui.View):
    ...
    async def show_position_menu(self, interaction: discord.Interaction):
        self.clear_items()
        btn_dealer = ui.Button(
            label="딜러 옵션", style=discord.ButtonStyle.danger, custom_id="dealer")
        btn_support = ui.Button(
            label="서폿 옵션", style=discord.ButtonStyle.primary, custom_id="support")
    ...
    async def show_option_list(self, interaction: discord.Interaction):
        ...
        opt_select = ui.Select(
            placeholder=f"2. {ACC_NAMES[acc_id]} {pos} 옵션 선택", options=select_options)
    ...
    async def show_value_list(self, interaction: discord.Interaction):
        ...
        val_select = ui.Select(
            placeholder=f"3. {main_opt} 수치 선택", options=val_options)
    ...
    async def show_second_option_list(self, interaction: discord.Interaction):
        ...
        select_options = [discord.SelectOption(
            label=opt, value=opt) for opt in filtered_options]
        second_opt_select = ui.Select(
            placeholder="4. 두 번째 옵션 선택", options=select_options)
    ...
    async def show_second_value_list(self, interaction: discord.Interaction):
        ...
        val_select = ui.Select(
            placeholder=f"5. {opt2_name} 수치 선택", options=val_options)
    ...
    async def show_final_step(self, interaction: discord.Interaction):
        ...
        picked = [self.search_data.get(
            'main_opt_name'), self.search_data.get('main_opt2_name')]
        ...
        final_type_select = ui.Select(placeholder="6. 마지막 세 번째 옵션 종류 선택", options=[
                                      discord.SelectOption(label=opt, value=opt) for opt in final_options])
    ...
    async def show_final_value_list(self, interaction: discord.Interaction):
        ...
        val_select = ui.Select(
            placeholder=f"7. {opt_name} 수치 선택", options=val_options)

        async def val_callback(inter: discord.Interaction):
            selected_text = val_select.values[0]
            # 숫자 저장
            self.search_data['option3_value'] = VALUE_MAPS[selected_text]

            # 최종 검색 버튼 등장
            search_btn = ui.Button(
                label="🔍 모든 조건으로 검색 시작", style=discord.ButtonStyle.success)
            ...
            async def search_click(i: discord.Interaction):
                ...
                try:
                    ...
                    embed.add_field(
                        name=d['main_opt_name'], value=f"{int(d['value1']) / 100:.2f}%", inline=True)
                    embed.add_field(
                        name=d['main_opt2_name'], value=f"{int(d['value2']) / 100:.2f}%", inline=True)
                    embed.add_field(
                        name=d['main_opt3_name'], value=f"{d['option3_value']}", inline=True)

                    # 3. 가격 정보 추가 (돋보이게 큰 필드로)
                    embed.add_field(
                        name="💰 최저가 정보", value=f"**{auction_acc['price']:,} G**", inline=False)
...
class AuctionCog(commands.Cog):
        ...
        if item_info:
            ...
            if price is not None and price > 0:
                if last_price is None or last_price == 0:
                    ...
                    change_emoji = "🔺"
                    # 계산 결과 뒤에 : , 를 붙여 콤마 추가
                    # {n:,} >> 천 단위 콤마 추가, {n:.2f} >> 소수점 둘째 자리까지 반올림, {n:,.0f} >> 콤마를 찍으면서 소수점 없앰
    ...
    async def auction_acc(self):
        ...
        deal_search_list = [
            ...
            # 목걸이 추피상 적주피상 공+상 (최저가)
            [200010, 15178, 41, 260, 42, 200, 53, 390],
            # ----------------------------------------------------------------------------------
            [200020, 13000, 45, 155, 46, 300, 53, 390],  # 귀걸이 공%상 무공%상 공+상
            # 귀걸이 공%상 무공%상 공+상 (최저가)
            [200020, 11806, 45, 155, 46, 300, 53, 390],
            # ----------------------------------------------------------------------------------
        ...
        heal_search_list = [
            ...
            # 귀걸이 공%상 무공+상 최생+상 (최저가)
            [200020, 11806, 46, 300, 54, 960, 55, 6500],
            # ----------------------------------------------------------------------------------
        ...
        for guild in self.bot.guilds:
            ...
            target_channel = discord.utils.get(
                guild.text_channels, name="로아-악세")
            if target_channel:
                try:
                    # 1. 임베드 객체 생성
    ...
    def generate_graph(self, df, title_suffix=""):
        ...
        for option, group in df.groupby('item_option'):
            ...
            plt.plot(group['created_at'], group['buy_price'],
